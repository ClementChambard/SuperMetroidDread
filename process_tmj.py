...
def tmj_object_property_get(tmj_obj: dict, name: str):
    if "properties" not in tmj_obj.keys():
        return 0
...
def map_collision_from_tmj(col_layer: dict) -> list[int]:
    dat = col_layer["data"]
...
def map_transition_from_tmj(tran_layer: dict) -> list[dict]:
...
def map_bloc_from_tmj(bloc_layer: dict) -> list[dict]:
...
def tmj_process(
    tmj_file: str, map_file: str, name: str = "UNNAMED", x: int = 0, y: int = 0
):
    ...
    if collisions != {}:
        out["collision"] = map_collision_from_tmj(collisions)

    transitions = tmj_layer_find(tmj, "transitions")
    if transitions != {}:
        out["transitions"] = map_transition_from_tmj(transitions)

    blocs = tmj_layer_find(tmj, "blocs")
    if blocs != {}:
        out["blocs"] = map_bloc_from_tmj(blocs)
...
def world_process(file_name: str):
    ...
    for m in maps:
        ...
        tmj_process(
            folder_name + tmj_file, folder_name + map_file, tmj_file, m["x"], -m["y"]
        )
