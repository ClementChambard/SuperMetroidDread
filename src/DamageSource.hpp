...
class DamageSource {
    public:
        ...
        void Activate(glm::vec2 position, float size, int dmg, Type type, uint8_t obj_id, int ttl = 99999);

        bool active() const { return flags & 1; }
        bool isBeam() const { return type <= Type::PLASMA_BEAM_DIFFUSION_EXPLODE; }
        bool isMissile() const { return type >= Type::MISSILE && type <= Type::STORM_MISSILE; }
