...
std::vector<int> Controller::BtnKbdMap = {
    NSK_lctrl, NSK_lalt, NSK_space, NSK_v,
    NSK_w, NSK_s, NSK_a, NSK_d,
    NSK_q, NSK_1, NSK_e, NSK_3,
    NSK_return, NSK_backspace,
    NSK_p, NSK_semicolon,
};

std::vector<int> Controller::Stick::LStiKbdMap = {
    NSK_up, NSK_down, NSK_left, NSK_right, NSK_lshift
};
std::vector<int> Controller::Stick::RStiKbdMap = {
    NSK_kp8, NSK_kp2, NSK_kp4, NSK_kp6, NSK_kp5
};
...
void Controller::Update(bool count)
{
    ...
    if (joystick == -1)
    {
        for (size_t i = 0; i < InputBuffer.size(); i++)
        {
            if (Inputs::Keyboard().Down(BtnKbdMap[i])) InputBuffer[i].hold = 0;
            if (Inputs::Keyboard().Pressed(BtnKbdMap[i])) InputBuffer[i].press = 0;
        }
    }
    else
    {
        for (size_t i = 0; i < InputBuffer.size(); i++)
        {
            if (Inputs::Gamepad(joystick).Down(i)) InputBuffer[i].hold = 0;
            if (Inputs::Gamepad(joystick).Pressed(i)) InputBuffer[i].press = 0;
        }
    }
}
...
{
    if (hold && InputBuffer[input].hold <= buff || !hold && InputBuffer[input].press <= buff)
    {
        if (del)
        {
            if (hold) InputBuffer[input].hold = 60; else InputBuffer[input].press = 60;
        }
        return true;
    }
    return false;
...
bool Controller::StickFlickX(bool isLeftStick, unsigned int buff)
{
    ...
    for (unsigned int i = 0; i < buff; i++)
        dist += st->flickedX[i];
    return abs(dist) > 0.5f && math::sign(dist) == -math::sign(st->sensibilities[0].x);
}
bool Controller::StickFlickY(bool isLeftStick, unsigned int buff)
{
    ...
    for (unsigned int i = 0; i < buff; i++)
        dist += st->flickedY[i];
    return abs(dist) > 0.5f && math::sign(dist) == -math::sign(st->sensibilities[0].y);
}
...
void Controller::Stick::cache_values(bool count)
{
    ...
    if (joystick == -1)
    {
        ...
        trueX = amt*127*(Inputs::Keyboard().Down(k[3])-Inputs::Keyboard().Down(k[2]));
        trueY = amt*127*(Inputs::Keyboard().Down(k[0])-Inputs::Keyboard().Down(k[1]));
        if (trueX != 0 && trueY != 0) { trueX /= sqrt(2); trueY /= sqrt(2); }
    }
    else
    {
    ...
    if (abs(trueX) < 22) trueX = 0;
    else trueX = fmin(abs(trueX)/120,1)*math::sign(trueX);
    if (abs(trueY) < 22) trueY = 0;
    else trueY = fmin(abs(trueY)/120,1)*math::sign(trueY);
...
void Controller::Stick::Draw(float x, float y, float r)
{
    ...
    NSEngine::draw_circle(x + r*sensibilities[0].x, y + r*sensibilities[0].y, r/3.f, false);
}
...
void ControllersManager::init()
{
    ...
    Inputs::InitGamepads();
    for (int i = 0; i < NSEngine::GamepadStruct::Number(); i++) controllers.push_back(new Controller(i));
}
...
{
    for (int i = 0; i < controllers.size(); i++) delete controllers[i];
    controllers.clear();
}
...
void ControllersManager::drawGui()
{
    if (NSEngine::getInstance()->flags().val&0b00000010)
        for (int i = 0; i < controllers.size(); i++)
            gui_draw_controller_widget(controllers[i], -480+i*400, 360, 2, c_white);
    else
        for (int i = 0; i < controllers.size(); i++)
            gui_draw_controller_widget(controllers[i], -600+i*100, 450, 0.5f, c_white);
}
