...
Samus::Samus() {
  ...
  // TODO(Me) : get starting position from save file
...
void Samus::applyAccelX(float xAccel, float xSpdMax, float xFriction,
                        bool turn_add) {
...
void Samus::applyAccelY(float yAccel, float ySpdMax, float yFriction,
                        bool turn_add) {
...
void playerState_Crouching(Samus *s) {
  ...
  if (s->inputs.dirUp.pressed)
    s->setState(PlayerState::IDLE);
  if (s->hasMorph &&
      (s->inputs.dirDo.pressed || s->inputs.morph.pressed && s->hasMorph))
    s->setState(PlayerState::MORPH);
...
void playerState_Airnormal(Samus *s) {
  ...
  s->applyAccelX(AIR_ACCEL_X * (s->inputs.dirRi - s->inputs.dirLe), AIR_SPEED_X,
                 AIR_FRICTION_X);
...
void playerState_Spinjump(Samus *s) {
  ...
  s->applyAccelX(SPIN_JUMP_SPEED_X * (s->faceR ? 1 : -1), SPIN_JUMP_SPEED_X,
                 abs(s->spd.x) > SPIN_JUMP_SPEED_X ? AIR_FRICTION_X : 0);
  if (s->state_timer > 1)
    s->applyAccelY(GRAVITY, AIR_SPEED_Y, AIR_FRICTION_Y);
  ...
  if (s->inputs.jump.pressed &&
      (s->hasSpaceJ || (s->hasSpinBo && !s->usedSpinboost)) && s->spd.y < 0) {
    s->usedSpinboost = true;
    s->setSpeedY(JUMP_FORCE);
    AnmManager::UpdateChild(AnmManager::newAnim(anmeffects_doubleJump),
                            s->col.bottom().x, s->col.bottom().y + 5, 0, 0, 0,
                            0, 1, 1, true);
  } else if (s->inputs.jump.pressed && s->col.isWallRight() && s->faceR) {
    s->setSpeedY(JUMP_FORCE);
    s->faceR = false;
    s->noMovementControlTimer = 29;
    AnmManager::UpdateChild(AnmManager::newAnim(anmeffects_wallJumpR),
                            s->col.right().x, s->col.right().y + 5, 0, 0, 0, 0,
                            1, 1, true);
  } else if (s->inputs.jump.pressed && s->col.isWallLeft() && !s->faceR) {
    s->setSpeedY(JUMP_FORCE);
    s->faceR = true;
    s->noMovementControlTimer = 29;
    AnmManager::UpdateChild(AnmManager::newAnim(anmeffects_wallJumpL),
                            s->col.left().x, s->col.left().y + 5, 0, 0, 0, 0, 1,
                            1, true);
  }
  ...
  if (s->hasScrew) {
    ...
    s->getDamageSource(ds).Activate(s->col.center(), 24, 1000,
                                    DamageSource::Type::SCREW_ATTACK, 255, 1);
  }
}

void playerState_Walk(Samus *s) {
  s->applyAccelY(GRAVITY, AIR_SPEED_Y, AIR_FRICTION_Y);
  s->applyAccelX(RUN_ACCEL * (s->inputs.dirRi - s->inputs.dirLe), RUN_SPEED,
                 GROUND_FRICTION, true);
...
// TODO(Me): Better calculation of the trajectory
void check_bombjump(Samus *s) {
...
void playerState_Morph(Samus *s) {
  s->applyAccelX(MORPH_ACCEL * (s->inputs.dirRi - s->inputs.dirLe), MORPH_SPEED,
                 MORPH_FRICTION);
...
void Samus::allowAim(AimDir allowedAim, AimDir defaultAim) {
  aim = getAim(inputs.dirLe, inputs.dirRi, inputs.dirUp, inputs.dirDo,
               inputs.dirAu, inputs.dirAd);
  if (!(aim & allowedAim))
    aim = defaultAim;
}
...
void Samus::allowPlaceBomb() {
  ...
  if ((hasCBombs || hasPBombs) && Inputs::Keyboard().Down(NSK_q))
    ...
    chargeCtr2 = 0;
  uint8_t typ = chargeCtr2 > 60                                ? 4
                : (!bomb_pool[5].isActive() && chargeCtr2) > 0 ? 2
                                                               : 0;
  if (!inputs.shoot.pressed)
    return;
  if (typ == 4) {
...
void Samus::Update() {
  ...
  // TODO(Me) : This is debug
  hasSpazer = Inputs::Keyboard().Down(NSK_a);

  // TODO(Me) : get shoot point correctly
...
void draw_tile(int x, int y, NSEngine::Color c) {
  ...
  if (yy < 0)
    yy -= 1;
  NSEngine::draw_rectangle_color(xx * 24, yy * 24, (xx + 1) * 24, (yy + 1) * 24,
                                 c, c, c, c);
}

// TODO(Me) : only for debug drawing, use anims otherwise
#include <Engine.hpp>
void Samus::Draw() {
  ...
  NSEngine::draw_quad_2d(col.topleft(), col.topright(), col.bottomright(),
                         col.bottomleft());
  ...
  std::string stateNames[] = {
      "Free",     "Idle",    "Crouching",       "Walking", "Airnormal",
      "Spinjump", "Sliding", "Slide_air_boost", "Morph",   "Morph_air",
  };

  auto vec2str = [](glm::vec2 v) -> std::string {
    return std::to_string(v.x) + ", " + std::to_string(v.y);
  };
  auto veci2str = [](glm::vec2 v) -> std::string {
    return std::to_string(static_cast<int>(v.x)) + ", " +
           std::to_string(static_cast<int>(v.y));
  };
  ...
  std::string timer_str =
      state_timer < 10000 ? std::to_string(state_timer) : "+9999";
  ...
  std::string state_str = stateNames[static_cast<size_t>(state)];
  font->renderText(state_str, glm::vec2(-270, 150), font_s);
  std::string pos_str = "p: " + veci2str(col.pos) +
                        " r: " + veci2str(Area::GetRoom()->getRoomPos(col.pos));
  ...
  std::string col_flags_str =
      "col: " + std::to_string(col.isGrounded()) +
      std::to_string(col.isClimbPos()) + std::to_string(col.isHardSlopeL()) +
      std::to_string(col.isHardSlopeR()) + std::to_string(col.isSlopeBelow()) +
      std::to_string(col.isWallLeft()) + std::to_string(col.isWallRight());
  font->renderText(col_flags_str, glm::vec2(-270, 140), font_s);
  snprintf(bufferForSprintf, sizeof(bufferForSprintf), "%2.1f fps",
           NSEngine::getInstance()->fps());
  font->set_halign(SpriteFont::align::RIGHT);
...
#define SHOOT_COLOR(t)                                                         \
  (t < 2 ? glm::vec3(255, 240, 0)                                              \
         : (t < 4 ? glm::vec3(32, 240, 16) : glm::vec3(240, 0, 255)))
...
void Samus::UpdateAnims() {
  for (int i = 0; i < MAX_ANIM; i++)
    if (anims[i] != -1) {
      ...
      AnmManager::UpdateChild(anims[i], pos().x + anmPositions[i].x,
                              pos().y + anmPositions[i].y, 0, 0, 0,
                              anmRotations[i], anmScales[i], anmScales[i], 1);
    }
}
