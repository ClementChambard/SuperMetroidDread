...
union AimDir {
    ...
    uint8_t dat;
    static constexpr float SHOOT_ANGLES[8] = { 0.0f, 0.7854f, 1.5708f, 2.3562f, 3.1416f, 3.927f, 4.7124f, 5.4978f };
...
public:
    ...
    void setState(PlayerState s)
    {
        state = s;
        state_timer = 0;
        if (state == PlayerState::SLIDING || state == PlayerState::SLIDE_AIR_BOOST || state == PlayerState::MORPH || state == PlayerState::MORPH_AIR)
            col.setSize(COL_SIZE_MORPH);
        else if (state == PlayerState::CROUCHING || state == PlayerState::SPINJUMP)
    ...
    void applyAccelX(float xAccel, float xSpdMax, float xFriction, bool turn_add = false);
    void applyAccelY(float yAccel, float ySpdMax, float yFriction, bool turn_add = false);
    void allowMovement(bool grounded = false);
    void allowShoot();
    void allowPlaceBomb();
    void allowCharge();
    void allowAim(AimDir allowedAim = AimDir_ALL, AimDir defaultAim = AimDir_FORWARD);
    ...
    int anims[MAX_ANIM]; // 0 = bottom, 1 = top, 2 = missile cap, 3, 4 = shoot anim / charge
    glm::vec2 anmPositions[MAX_ANIM];
    float anmRotations[MAX_ANIM];
    float anmScales[MAX_ANIM];
    void SetAnim(int id, int anm);
    void SetAnim(int id, int anm, glm::vec2 relPos, float rot = 0, float scale = 1);
    DamageSource& getDamageSource(int id) { return damage_sources[id]; }
    int allocDmgSrc() { static int cur = 0; int i = 0; while (i++ < MAX_DAMAGE_SOURCE && damage_sources[cur].active()) cur = (cur+1)%MAX_DAMAGE_SOURCE; return cur; }
    void UpdateAnims();
};
