...
Camera* Camera::GameCamera() {
    if (!gameCamera) gameCamera = new Camera();
    return gameCamera;
}
...
Camera::Camera() {
    ...
    this->z = NSEngine::getInstance()->window().getWindowData().bheight / 0.828427f;
}
Camera::~Camera() {}
...
void Camera::SetPos(float x, float y)
{
    ...
    else y = math::clamp(y, miny, maxy);
    NSEngine::activeCamera3D()->setPosition(x, y, z);
}


void Camera::GetViewSize(float& w, float& h)
{
    h = 0.828427f * this->z;
    auto d = NSEngine::getInstance()->window().getWindowData();
    w = h * d.bwidth / (float)d.bheight;
}
