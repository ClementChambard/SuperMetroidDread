...
{
    auto tilebelow = Area::GetRoom()->getCol()->getTileAtGlobalPos(pos.x, pos.y - 1);
...
Collider& Collider::Update(glm::vec2& speed)
{
    ...
    speedFraction = actualSpeed - glm::vec2(floor(actualSpeed.x), floor(actualSpeed.y));
    actualSpeed -= speedFraction;
    ...
    if (dirx > 0)
        while (actualSpeed.x != i++) {
            pos.x += dirx;
            if (!colmap->getTileAtGlobalPos(topright()) || (!onSlope && !colmap->getTileAtGlobalPos(bottomright())) || !colmap->getTileAtGlobalPos(right())) {
                speed.x = 0;
                speedFraction.x = 0;
                pos.x--;
                break;
        ...
        while (actualSpeed.x != i--) {
            pos.x += dirx;
            if (!colmap->getTileAtGlobalPos(topleft()) || (!onSlope && !colmap->getTileAtGlobalPos(bottomleft())) || !colmap->getTileAtGlobalPos(left())) {
                speed.x = 0;
                speedFraction.x = 0;
                pos.x++;
                break;
    ...
    grounded = false;
    bool slopeMode = onSlope || (colmap->getTileAtGlobalPos(bottomleft()) % 16) || (colmap->getTileAtGlobalPos(bottomright()) % 16);
    while (collideAt(pos) || !slopeMode && (collideAt(bottomleft()) || collideAt(bottomright()))) {
        pos.y++;
        if (actualSpeed.y < 0) {
            speed.y = 0;
            speedFraction.y = 0;
    ...
    canClimb = actualSpeed.y >= 0 && !onSlope && (!wallLeft && collideAt(ble1) || !wallRight && collideAt(bri1));
