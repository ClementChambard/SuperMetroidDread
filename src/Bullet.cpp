...
void Bullet::Launch(const glm::vec2& p, float d, float s, char t)
{
    ...
    Samus::instance->getDamageSource(dmg_src).Activate(pos, 3, 1, DamageSource::Type::POWER_BEAM, id);
}
...
void Bullet::Update()
{
    ...
    if (Collider::collideAt(pos))
    {
        ...
        while (Collider::collideAt(pos)) pos -= sss;
        Break();
    }
}
...
void Missile::Launch(const glm::vec2& p, float angle, uint8_t flags)
{
    ...
    dmg_src = Samus::instance->allocDmgSrc();
    Samus::instance->getDamageSource(dmg_src).Activate(pos, 3, 1, DamageSource::Type::MISSILE, id);
}
...
void Missile::Update()
{
    ...
    if (lifetime > 10 && lifetime % 8 == 0) AnmManager::UpdateChild(AnmManager::newAnim(anmeffects_missileSmoke), pos.x-spd.x*2, pos.y-spd.y*2, 0, 0, 0, angle, 1, 1, true);
    //check hit with enemy
...
void Bomb::Update() {
    ...
    if (lifetime >= 60) {
        dmg_src = Samus::instance->allocDmgSrc();
        Samus::instance->getDamageSource(dmg_src).Activate(pos, 14, 1, DamageSource::Type::BOMB, id, 3);
        flags = 0;
        AnmManager::deleteAnim(anmID);
        // explosion anm
    }
