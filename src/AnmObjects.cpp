...
void AnmObjects::Init() {
  int t =
      NSEngine::TextureManager::RegisterTexture("assets/textures/objects.png");
...
static void anmobject_blocbreak(AnimScript *anm) {
  if (anm->frame == 0) {
    ...
    anm->set_rot(Random::Floatm11() * 0.2, Random::Floatm11() * 0.2,
                 Random::Angle());
    anm->rot_vel(Random::Floatm11() * 0.02, Random::Floatm11() * 0.02,
                 Random::Angle() / 4.f);
    anm->set_scale(Random::Float01() * 0.2 + 0.8,
                   Random::Float01() * 0.2 + 0.8);
    anm->scale_time(15, 4, Random::Float01() * 0.1 + 0.6,
                    Random::Float01() * 0.1 + 0.6);
  }
  if (anm->frame == 15)
    anm->destroy();
}
...
static void anmobject_blocscr(AnimScript *anm) {
  ...
  if (anm->interrupt == 1) {
    anm->set_alpha(0);
    AnmManager::anim(AnmManager::newAnim(anmobject_blocbreak, -1, false))
        ->set_pos(anm->x_(1), anm->y_(1), 0)
        ->set_sprite(s_blocpiece1);
    AnmManager::anim(AnmManager::newAnim(anmobject_blocbreak, -1, false))
        ->set_pos(anm->x_(1), anm->y_(1), 0)
        ->set_sprite(s_blocpiece2);
    AnmManager::anim(AnmManager::newAnim(anmobject_blocbreak, -1, false))
        ->set_pos(anm->x_(1), anm->y_(1), 0)
        ->set_sprite(s_blocpiece3);
    AnmManager::anim(AnmManager::newAnim(anmobject_blocbreak, -1, false))
        ->set_pos(anm->x_(1), anm->y_(1), 0)
        ->set_sprite(s_blocpiece4);
...
uint16_t anmobject_block(int type, int x, int y, int interrupt) {
  ...
  if (type == 0) {
    AnmManager::anim(anm)->set_sprite(s_blocbeam);
  } else if (type == 1) {
