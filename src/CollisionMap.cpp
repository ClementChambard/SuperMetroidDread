...
void CollisionMap::fromJsonArray(Json::Value const& val)
{
    ...
    for (int i = 0; i < width*height; i++) v.push_back(val["collision"][i].asInt());
    tiles = v;//getTiles(v);
}
...
{
    int texID = NSEngine::TextureManager::RegisterTexture("assets/textures/Collisions.png");
...
bool CollisionMap::collideWith(uint8_t tile, float xx, float yy)
{
    switch (tile) {
        ...
        case 13:
            return tilesize - yy < (-xx + 1 * tilesize) / 3.f;
        case 17:
            if (xx >= tilesize-1) return true; // TODO: small correction : should not be needed
...
{
    alterations.erase(remove(alterations.begin(), alterations.end(), a), alterations.end());
    //std::cout << "unalter\n";
}

bool CollisionMap::collideAt(float x, float y) { uint8_t t; return collideAt(x, y, t); }
...
uint8_t CollisionMap::getTileAt(float x, float y) { float xx, yy; return getTileAt(x, y, xx, yy); }
uint8_t CollisionMap::getTileAt(float x, float y, float& xx, float& yy)
{
    xx = abs((int) x % (int) tilesize);
    yy = abs((int) y % (int) tilesize);
    if (x < 0 || y < 0 || x > width*tilesize || y > height*tilesize) return 16;
    int row = floor(y/tilesize);
    int column = floor(x/tilesize);

    for (auto a : alterations)
    {
        if (row < a->starty || row >= a->starty + a->h || column < a->startx || column >= a->startx + a->w) continue;
    ...
    return tiles[thetile]; //(tiles[thetile / tileperblock] >> ((thetile % tileperblock) * 5) ) & 0b11111;
}
...
void CollisionMap::draw(glm::vec2 worldpos)
{
    ...
    {
        if (i % width == 0 && i != 0) { x = 0; y += tilesize; }
        if (y >= tilesize * height) break;
        uint8_t t = getTileAt(x, y);
        if (t != 16)
            NSEngine::draw_game_sprite(colsprite0 + t, worldpos.x+x +0.5*tilesize, worldpos.y-y -0.5*tilesize, 1, 1, 0, c_white, c_white, c_white, c_white);
        x += tilesize;
    }
}
