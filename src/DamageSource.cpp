...
void DamageSource::Activate(glm::vec2 position, float size, int dmg, Type type,
                            uint8_t obj_id, int ttl) {
  this->pos = position;
  this->damage = dmg;
  this->size = size;
  this->type = type;
...
bool DamageSource::intersectCircle(glm::vec2 pos, float size) const {
  return (pos.x - this->pos.x) * (pos.x - this->pos.x) +
         ...
         (pos.x + spd.x - this->pos.x) * (pos.x + spd.x - this->pos.x) +
                 (pos.y + spd.y - this->pos.y) * (pos.y + spd.y - this->pos.y) <
             (size + this->size) * (size + this->size);
}
...
bool DamageSource::intersectRectangle(glm::vec2 pos1, glm::vec2 pos2) const {
  ...
  // distance
  bool b1 =
      (pos.x - xx) * (pos.x - xx) + (pos.y - yy) * (pos.y - yy) < size * size;
  if (b1)
    return true;
  ...
  return (pos.x + spd.x - xx) * (pos.x + spd.x - xx) +
             (pos.y + spd.y - yy) * (pos.y + spd.y - yy) <
         size * size;
}
...
void DamageSource::Draw() const {
  NSEngine::draw_circle_color(pos.x, pos.y, size, {255, 0, 0, 128},
                              {255, 0, 0, 128});
  NSEngine::draw_circle_color(pos.x + spd.x, pos.y + spd.y, size,
                              {255, 0, 0, 64}, {255, 0, 0, 64});
}
