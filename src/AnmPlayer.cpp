...
void AnmPlayer::Init() {
  int tex =
      NSEngine::TextureManager::RegisterTexture("assets/textures/beams.png");
...
void PlayerBeamFireN(AnimScript *anm) {
  if (anm->frame == 0) {
    ...
    anm->set_sprite(plSprBeamFireN2);
  } else if (anm->frame == 4) {
    anm->set_sprite(plSprBeamFireN3);
  } else if (anm->frame == 6) {
    anm->destroy();
  }
}
...
void PlayerBeamFireC(AnimScript *anm) {
  if (anm->frame == 0) {
    ...
    anm->set_sprite(plSprBeamFireC2);
  } else if (anm->frame == 4) {
    anm->set_sprite(plSprBeamFireC3);
  } else if (anm->frame == 6) {
    anm->destroy();
  }
}
