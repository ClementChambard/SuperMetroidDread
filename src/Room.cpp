...
void Room::Update()
{
    for (auto& d : doors) d.Update(Samus::instance);
    for (auto& b : blocs) b.Update();
}
...
void Room::LoadFromJson(std::string fileName)
{
    ...
    for (auto t : val["transitions"]) {
        transitions.push_back({{t["x1"].asInt(), t["y1"].asInt()}, {t["x2"].asInt(), t["y2"].asInt()}, (unsigned char)t["dest"].asInt(), (unsigned char)t["flags"].asInt()});
    }
...
void Room::LoadMetaFromJson(Json::Value const& val) {
    ...
    for (auto b : val["blocs"]) {
        blocs.emplace_back((BlockType)b["t"].asInt(), b["x"].asInt(), b["y"].asInt(), &collision);
    }
}
...
void Room::Draw()
{
    ...
    for (auto& b : blocs) b.Draw();
    //worldPos *= glm::vec2(1,-1);
    //for (auto t : transitions)
    //{
    //    glm::vec2 pos1 = worldPos + t.pos1;
    //    glm::vec2 pos2 = worldPos + t.pos2;
    //    NSEngine::draw_rectangle_color(pos1.x,-pos1.y,pos2.x,-pos2.y, c_white, c_white, c_white, c_white);
    //}
    //worldPos *= glm::vec2(1,-1);
}
...
void moveForTransition(RoomTransition const& transition, Collider* c)
{
    ...
    switch (dir) {
        case 0: c->setPos(c->bottom()+glm::vec2(-w*2,0)); break;
        case 1: c->setPos(c->bottom()+glm::vec2(w*2,0)); break;
        case 2: c->setPos(c->bottom()+glm::vec2(0, c->h())); break;
        case 3: c->setPos(c->bottom()+glm::vec2(0,-c->h())); break;
    }
}
...
void Room::checkRoomTransition(Collider* col) const
{
    ...
    for (auto t : transitions)
    {
        if (c1.x > t.pos2.x || c2.x < t.pos1.x || c2.y < t.pos1.y || c1.y > t.pos2.y) continue;
